...
class BasicLayer(nn.Module):
    ...
    def forward(self, x): # FIXME i think i need to first up(downsample) for reverse
        if self.downsample is not None and self.downsample.operation_type == "upsample": # upsample before attention, downsample after attention
            x = self.downsample(x)
    ...
    def extra_repr(self) -> str:
        return f"input_resolution={self.input_resolution}, dim={self.dim}, depth={self.depth}, num_heads={self.num_heads}, window_size={self.window_size}"
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.operation_type = "downsample"
...
class PatchSeperating(nn.Module):
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim # dim = C
...
class SwinTransformerV21D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # out_parameters
        self.out_channels = int(embed_dim * 2 ** (self.num_layers - 1))
        self.out_resolution = patches_resolution // (2 ** (self.num_layers - 1))
        ...
        # self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity() # TODO change to get pixelwise class
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
        x = self.norm(x)  # B L C
        # x = self.avgpool(x.transpose(1, 2))  # B C 1 # needed only for reverse
        # x = torch.flatten(x, 1)
...
class SwinTransformerV21D_reverse(nn.Module): # TODO write reverse so it works on its own and doesnt rely on symmetry. Do for loop normal direction and change everything so it works with variable input and output shapes.
    # currently it relies on the forward swin to be the same size and shape
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # out_parameters
        self.out_channels = self.num_classes // (2 ** (self.num_layers - 1))
        self.out_resolution = patches_resolution * (2 ** (self.num_layers - 1))
        ...
        for i_layer in range(self.num_layers-1,-1,-1):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                               norm_layer=norm_layer,
                               downsample=PatchSeperating if (i_layer < self.num_layers - 1) else None, # patch merging always except for last layer
                               use_checkpoint=use_checkpoint,
                               pretrained_window_size=pretrained_window_sizes[i_layer])
            self.layers.append(layer)
...
class U_swintransformer(nn.Module):
    def __init__(self,
        ...
        self.enc = SwinTransformerV21D(img_size=img_size,
        ...
        print("Middle resolution: ", self.middle_resolution)
        print("Middle dim: ", self.middle_dim)

        self.dec = SwinTransformerV21D_reverse(img_size=img_size,
            patch_size=patch_size,
            in_chans=in_chans,
            num_classes=num_classes,
            embed_dim=embed_dim,
...
if __name__ == "__main__":
    swin_transformer = SwinTransformerV21D(img_size=256,
                                               patch_size=4,
                                       in_chans=1,
                                       depths=[2, 2, 6, 2],
                                       num_heads=[3, 6, 12, 24],
