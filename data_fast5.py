...
class H5Dataset_10(torch.utils.data.Dataset):
    def __init__(self, path):
        self.file_path = path
        self.dataset = None
    ...
    def __getitem__(self, index):
        ...
        signal = np.array([])
        for i in range(10):
            signal = np.append(signal,self.dataset[self.segment_names[index*10+i]][:])

        return torch.from_numpy(signal).float().unsqueeze(0)# unsqueeze: add channel axis

    def __len__(self):
        return (len(self.segment_names)-10)//10
...
if __name__ == "__main__":
    ...
    print(len(h5py.File("/work/zo48kij/data_masters/data_segments_reduced.h5", "r")))
